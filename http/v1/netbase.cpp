...
enum Network ParseNetwork(const std::string& net_in) {
    std::string net = net_in;//ToLower(net_in);
    if (net == "ipv4") return NET_IPV4;
    if (net == "ipv6") return NET_IPV6;
    if (net == "onion") return NET_ONION;
    if (net == "tor") {
        printf("Warning: net name 'tor' is deprecated and will be removed in the future. You should use 'onion' instead.\n");
        return NET_ONION;
    }
...
static IntrRecvError InterruptibleRecv(uint8_t* data, size_t len, int timeout, const SOCKET& hSocket)
{
    ...
    int64_t curTime = duration_in_ms.count();
    ...
    while (len > 0 && curTime < endTime) {
	    ...
	    duration_in_ms = std::chrono::duration_cast<std::chrono::milliseconds>(time_now.time_since_epoch());
        curTime = duration_in_ms.count();
...
static bool Socks5(const std::string& strDest, int port, const ProxyCredentials *auth, const SOCKET& hSocket)
{
    ...
    if ((recvr = InterruptibleRecv(pchRet1, 2, SOCKS5_RECV_TIMEOUT, hSocket)) != IntrRecvError::OK) {
        printf("Socks5() connect to %s:%d failed: InterruptibleRecv() timeout or other failure\n", strDest.c_str(), port);
        return false;
    }
    ...
    if (pchRet1[1] == SOCKS5Method::USER_PASS && auth) {
        ...
        printf( "SOCKS5 sending proxy authentication %s:%s\n", auth->username.c_str(), auth->password.c_str());
        uint8_t pchRetA[2];
        ...
        return error("Proxy requested wrong authentication method");
    }
    ...
    if (pchRet2[1] != SOCKS5Reply::SUCCEEDED) {
        ...
        printf("Socks5() connect to %s:%d failed: %s\n", strDest.c_str(), port, Socks5ErrorString(pchRet2[1]).c_str());
        return false;
    }
...
SOCKET CreateSocket(const CService &addrConnect)
{
    ...
    if (!addrConnect.GetSockAddr((struct sockaddr*)&sockaddr, &len)) {
        printf("Cannot create socket for %s: unsupported network\n", addrConnect.ToString().c_str());
    ...
    if (!SetSocketNonBlocking(hSocket, true)) {
        CloseSocket(hSocket);
        printf("CreateSocket: Setting socket to non-blocking failed, error %s\n", NetworkErrorString(WSAGetLastError()).c_str());
    }
    return hSocket;
}
...
bool ConnectSocketDirectly(const CService &addrConnect, const SOCKET& hSocket, int nTimeout, bool manual_connection)
{
    ...
    socklen_t len = sizeof(sockaddr);
    if (hSocket == INVALID_SOCKET) {
        printf("Cannot connect to %s: invalid socket\n", addrConnect.ToString().c_str());
        return false;
    }
    if (!addrConnect.GetSockAddr((struct sockaddr*)&sockaddr, &len)) {
    ...
    if (connect(hSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEINPROGRESS || nErr == WSAEWOULDBLOCK || nErr == WSAEINVAL)
        {
            ...
            if (nRet == SOCKET_ERROR)
            {
                printf("select() for %s failed: %s\n", addrConnect.ToString().c_str(), NetworkErrorString(WSAGetLastError()).c_str());
                return false;
            }
            ...
            {
                printf("getsockopt() for %s failed: %s\n", addrConnect.ToString().c_str(), NetworkErrorString(WSAGetLastError()).c_str());
                return false;
            }
            if (nRet != 0)
            {
                printf("connect() to %s failed after select(): %s", addrConnect.ToString().c_str(), NetworkErrorString(nRet).c_str());
                return false;
            }
        }
#ifdef WIN32
...
#endif
        {
            printf("connect() to %s failed: %s", addrConnect.ToString().c_str(), NetworkErrorString(WSAGetLastError()).c_str());
            return false;
        }
    }
    return true;
...
bool ConnectThroughProxy(const proxyType &proxy, const std::string& strDest, int port, const SOCKET& hSocket, int nTimeout, bool *outProxyConnectionFailed)
{
    ...
    if (proxy.randomize_credentials) {
        ...
        random_auth.username = random_auth.password = std::to_string(counter++);
...
std::string NetworkErrorString(int err)
{
    ...
    if (strerror_r(err, buf, sizeof(buf)))
        buf[0] = 0;
#endif
    return std::string(s) + " (" + std::to_string(err) + ")";
...
#endif
    if (ret) {
        printf("Socket close failed: %d. Error: %s\n", hSocket, NetworkErrorString(WSAGetLastError()).c_str());
    }
