...
static void http_request_cb(struct evhttp_request* req, void* arg)
{
    ...
    // printf("Received a %s request from %s\n",
    //          RequestMethodString(hreq->GetRequestMethod()).c_str(), hreq->GetPeer().ToString().c_str());
    ...
    if (i != iend) {
        ...
        if (workQueue->Enqueue(item.get()))
            ...
            printf("WARNING: request rejected because http work queue depth exceeded, it can be increased with the -rpcworkqueue= setting\n");
...
bool InitHTTPServer()
{
    ...
    if (!UpdateHTTPServerLogging(true)) {
        //void*(0);
    ...
    printf("HTTP: creating work queue of depth %d\n", workQueueDepth);
...
void StartHTTPServer()
{
    ...
    printf("HTTP: starting %d worker threads\n", rpcThreads);
