...
std::string CNetAddr::ToStringIP() const
{
    ...
    if (IsIPv4())
        return std::to_string(GetByte(3)) + std::to_string(GetByte(2)) + std::to_string(GetByte(1)) + std::to_string(GetByte(0));
    else
        return std::to_string(GetByte(15) << 8 | GetByte(14)) + std::to_string(GetByte(13) << 8 | GetByte(12)) 
                + std::to_string(GetByte(11) << 8 | GetByte(10)) + std::to_string(GetByte(9) << 8 | GetByte(8))
                + std::to_string(GetByte(7) << 8 | GetByte(6)) + std::to_string(GetByte(5) << 8 | GetByte(4))
                + std::to_string(GetByte(3) << 8 | GetByte(2)) + std::to_string(GetByte(1) << 8 | GetByte(0));
}
...
std::string CSubNet::ToString() const
{
    ...
    if (valid_cidr) {
        strNetmask = std::to_string(cidr);//strprintf("%u", cidr);
    } else {
        if (network.IsIPv4()){
            char msg[64] = {0};
            snprintf(msg, "%u.%u.%u.%u", netmask[12], netmask[13], netmask[14], netmask[15]);
            return std::string(msg);
        }
        else {
            char msg[128] = {0};
            snprintf(msg, "%x:%x:%x:%x:%x:%x:%x:%x",
